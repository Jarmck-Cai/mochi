...
- **⚠️ rime.dll 部署待 UAC**：局部候选的新 rime.dll 已编好，等管理员确认替换（`experiments\004-weasel-deploy\deploy-weasel.ps1` 提权运行）。部署后把 brain 从 `--no-prefix` 切回正常启动（重启去掉该旗标）。**当前 brain 以 --no-prefix 运行**（新 brain+旧插件窗口期前缀候选会吃掉剩余拼音，安全闸）
